...
const PostPage = () => {
    ...
    let { title, content, des, bannerUrl, author: { personal_info: { fullname, username: author_username , profile_img } }, publishedAt } = post;
    ...
    const fetchPost = () => {
        ...
        .then(async ({ data: { post } }) => {
            ...
            axios.post(import.meta.env.VITE_NODE_SERVER_DOMAIN + "/search-posts", { tag: post.tags[0], limit: 6, eliminate_post: post_id })
            .then(({ data }) => {
    ...
    return (
        <>
            <Helmet>
                <title>christisking | {title}</title>
