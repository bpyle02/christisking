...
const NotificationCard = ({ data, index, notificationState }) => {
    ...
    let { seen, type, reply, createdAt, comment, replied_on_comment, user, user: { personal_info: { fullname, username, profile_img } }, post: { _id , post_id, title }, _id: notification_id } = data;
    ...
    const handleDelete = (comment_id, type, target) => {
        ...
        axios.post(import.meta.env.VITE_NODE_SERVER_DOMAIN + "/delete-comment", { _id: comment_id }, {
            headers: {
    ...
    return (
        <div className={"p-6 border-b border-grey border-l-black " + ( !seen ? "border-l-2" : "" )}>
            <div className="flex gap-5 mb-3">
                <img src={profile_img} referrerPolicy="no-referrer" className="w-14 h-14 flex-none rounded-full" />
                <div className="w-full">
...

                    {
                        ...
                        : 
                        <Link to={`/post/${post_id}`} className="font-medium text-dark-grey hover:underline line-clamp-1">{`"${title}"`}</Link>
...

            {
                ...
                isReplying ? 
                <div className="mt-8">
                    <NotificationCommentField _id={_id} post_author={user} index={index} replyingTo={comment._id} setReplying={setReplying} notification_id={notification_id} notificationData={notificationState} />
                </div> : ""
